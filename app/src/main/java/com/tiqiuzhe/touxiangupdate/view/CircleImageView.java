...
//    会发现第一次进入时，第一个if语句为真，进入括号体内设置mSetupPending为true然后直接返回，后面的代码并没有执行。
// 为什么要这样设置mReady和mSetupPending呢不执行下面的代码呢，不要急，后面再解释。
    ...
    private void setup() {
        ...
        // 查看xml也就是160dp(240px)  getWidth得到是某个view的实际尺寸
        mBorderRect.set(0, 0, getWidth(), getHeight());
        //计算 圆形带边界部分（外圆）的最小半径，
        // 取mBorderRect的宽高减去一个边缘大小的一半的较小值
        // （这个地方我比较纳闷为什么求外圆半径需要先减去一个边缘大小）
...
//    目的是用最小的缩放比例，使得图片的某个方向的边的尺寸缩放到图片显示区域（mDrawableRect）一样。
// 做到了图片损失度最小。同时scale保证Bitmap的宽或高和目标区域一致，那么高或宽就需要进行位移，使得Bitmap居中。这里写得很nice。
